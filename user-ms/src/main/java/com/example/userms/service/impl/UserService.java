...
public class UserService implements IUserService {
    ...
    public ResponseEntity<UserResponseDto> getUser(String name) {
        Optional<User> userByUsername = userRepository.findUserByUsername(name);
    ...
    public ResponseEntity<String> saveUser(UserRequestDto request) {
        ...
        Optional<User> userByUsernameOrEmail = userRepository.findUserByUsernameOrEmail(request.getUsername().trim());
...
//
//        kafkaTemplate.send("create-rank-topic",kafkaRequest);KafkaRequest kafkaRequest = KafkaRequest.builder()
...
////                .build();
    ...
    public ResponseEntity<AuthenticationResponse> authenticateUser(AuthenticationRequest request) {
        log.info("authenticateUser method is working...");
        try {
    ...
    public ResponseEntity<String> sendOTP(String token) {
        ...
        if (!jwtService.isTokenExpired(jwt)) {
            ...
            KafkaRequest kafkaRequest = KafkaRequest.builder()
                    .email(user.orElseThrow(() -> new ConflictException("Token_NOT_FOUND")).getEmail())
                    .token(otp)
                    .build();
    ...
    private String createAndSaveOtp(Optional<User> user) {
        ...
        ConfirmationToken confirmationToken = ConfirmationToken.builder()
                .token(otp)
                .user(user.orElseThrow(() -> new ConflictException("USER_NOT_FOUND")))
    ...
    public ResponseEntity<String> checkOtp(String username,String otp) {
        Optional<User> user = userRepository.findUserByUsernameOrEmail(username);
        if (user.isEmpty()){
            throw new ConflictException("USERNAME_NOT_FOUND");
        }
    ...
    public ResponseEntity<String> checkEmailInDatabase(String email){
        ...
        if (user.isEmpty()){
            log.info("User from database -> {}",user.orElseThrow(() -> new ConflictException("USER_NOT_FOUND"))
                    .getUsername());
    ...
    public AuthenticationResponse refreshToken(String token) {
        if (!securityHelper.authHeaderIsValid(token)){
            throw new ConflictException("TOKEN_IS_WRONG");
        }
        ...
        if (username != null){
            if (userByName.orElseThrow(() -> new ConflictException("USER_NOT_FOUND"))
            ...
            throw new ConflictException("USERNAME_IS_NULL");
        }
        return null;
    }
    ...
    public ResponseEntity<String> confirmAccount(String token) {
        ...
        Optional<User> user = userRepository.findUserByUsernameOrEmail(confirmationToken.orElseThrow(() -> new RuntimeException("Token not found!"))
                .getUser().getUsername());
        user.orElseThrow(() -> new ConflictException("USER_NOT_FOUND"))
                .setEnabled(true);
